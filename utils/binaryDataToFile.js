...
export const binaryDataToFile = async (binaryData) => {
	if (!(binaryData instanceof Buffer)) {
		throw new Error("Expected binary data to be a Buffer");
	}

	const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "audio-"));

	// Detect file type
	const type = await fileTypeFromBuffer(binaryData);
	...
	await fs.writeFile(originalFilePath, binaryData);

	console.log(
		`Original file type: ${type.ext}, size: ${binaryData.length} bytes`
	);
